...
if (!function_exists('getAreaParentsRange')) {
	function getAreaParentsRange($value = 0, $areaName = null) {
		return getAreaParents($value, $areaName, '6212%');
	}
}
...
if (!function_exists('getAllParentStr')) {
	function getAllParentStr($value = 0, $areaName = null) {
		$parents = getAreaParents($value, $areaName, '6212%');
		if (!$parents) {
			return '';
		}
		$str = '';
...
if (!function_exists('getAllParentId')) {
	function getAllParentId($value = 0, $areaName = null) {
		$parents = getAreaParents($value, $areaName, '6212%');
		...
		$str = $parents[0]['pid'] . $str;
		return rtrim($str, ',');
	}
}

if (!function_exists('getAllParentPid')) {
	function getAllParentPid($value = 0, $areaName = null) {
		$parents = getAreaParents($value, $areaName, '6212%');
		...
		$str = $str . end($parents)['id'];
		return rtrim($str, ',');
	}
}
if (!function_exists('deSql')) {
	/**
	 * 解密SQL
	 */
	function deSql($sql) {
		return Crypt::decode($sql);
	}
}

if (!function_exists('enSql')) {
	/**
	 * 加密SQL
	 */
	function enSql($sql) {
		return Crypt::encode($sql);
	}
}

if (!function_exists('getAreaList')) {
	/**
	 * 获取地区列表
...
if (!function_exists('getAreaParents')) {
	/**
	 * 获取指定区域父级区域
	 * @param string $value 地区id
	 * @param string $key 获取指定字段
	 * @return array
	 */
	function getAreaParents($value = 0, $areaName = null, $startRange = false) {
		$stepless_area = Db::name('area_stepless');
		if ($startRange) {
			$stepless_area->whereLike('id', $startRange);
		}

		$stepless_area = $stepless_area->column('id,pid,name');

		$Parents = Tree::getParents($stepless_area, $value);
		$ParentsName = [];
		foreach ($Parents as $name => $Parent) {
			if ($areaName == null) {
				$ParentsName[] = $Parent;
			} else {
				$ParentsName[] = $Parent[$areaName];
			}
		}
		return $ParentsName;
	}
...
if (!function_exists('getDbConnectionConfig')) {
	 ...
	 * @return array
	 */
	function getDbConnectionConfig(string $key = null) {
		$config = Db::getConfig();
		if ($key && isset($config['connections'][$config['default']][$key])) {
			return $config['connections'][$config['default']][$key];
		} else {
...
if (!function_exists('snakeToCamel')) {
	 ...
	 * @param bool $capitalized 首字母是否大写
	 * @return string
	 */
...
if (!function_exists('uuid')) {
	...
	function uuid($length = 8): string {
		...
		// 按指定长度分隔唯一ID
		$splitId = implode('-', str_split(substr($sha1Hash, 0, $length), $length));
...
if (!function_exists('mergeEnvFiles')) {
	function mergeEnvFiles($dir, $filename) {
		...
		foreach ($items as $item) {
			...
			if (is_dir($path)) {
				$subConfig = mergeEnvFiles($path, $filename);
				$config = array_merge_recursive($config, $subConfig);
			} elseif (is_file($path) && $item == $filename) {

				$config = array_merge_recursive($config, include $path);
			}
		}
		return $config;
	}
...
if (!function_exists('getApiDocConfig')) {
	function getApiDocConfig() {
		return mergeEnvFiles(root_path() . 'app/', 'apidoc.php');
		$apidoc_config = Cache::get('apidoc_config');
		if (!$apidoc_config) {
			$apidoc_config = mergeEnvFiles(root_path() . 'app/', 'apidoc.php');
			Cache::set('apidoc_config', $apidoc_config);
		}
		return $apidoc_config;
	}
}
...
if (!function_exists('getAppConfig')) {
	 ...
	 * 获取应用目录内指定配置文件
	 */
	function getAppConfigOfFile($filePath = '') {
		return mergeEnvFiles(root_path() . 'app/', $filePath);
		$apidoc_config = Cache::get(md5($filePath));

		if (!$apidoc_config) {
			$apidoc_config = mergeEnvFiles(root_path() . 'app/', $filePath);
			Cache::set(md5($filePath), $apidoc_config);
		}
		return $apidoc_config;
	}
}
...
if (!function_exists('getAppConfig')) {
	function model($name = '') {
		list($AppName, $className) = explode('/', $name);
		$class = 'app\\' . $AppName . '\\model\\' . ucfirst($className);
...
// 加载自定义公共文件
// if (is_file(root_path() . 'app/function.php')) {
// 	include_once root_path() . 'app/function.php';
// }
...
if (!function_exists('event')) {
	 ...
	 * @alter 小乌 <82950492@qq.com>
	 */
	function event($name = '', $params = null, $once = false) {
	...
	function module_config($name = '', $value = '') {
		if ($name === '') {
			...
			if (strpos($name, '.')) {
				...
				return Module::getConfig($name, $item);
			} else {
				return Module::getConfig($name);
			}
		} else {
			// 设置值
			return Module::setConfig($name, $value);
		}
	}
}
	...
	function get_plugin_class($name) {
		...
		if (class_exists($class)) {
			...
			foreach ($modules as $module) {
				...
				$module_plugin_dir = root_path() . 'app' . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR . 'plugin';

				if (is_dir($module_plugin_dir)) {

					$dir_files = scandir($module_plugin_dir);

					foreach ($dir_files as $key => $dir_file) {

						if (is_file($module_plugin_dir . DIRECTORY_SEPARATOR . $dir_file)) {

							$class = "app\\{$module}\\plugin\\{$name}";
...
if (!function_exists('format_moment')) {
	 ...
	 * @return false|string
	 */
	function format_moment(string $time = null, $format = 'YYYY-MM-DD HH:mm') {
		$format_map = [
			// 年、月、日
			'YYYY' => 'Y',
			'YY' => 'y',
...
if (!function_exists('get_level_data')) {
	...
	function get_level_data($table = '', $pid = 0, $pid_field = 'pid') {
		...
		$data_list = Db::name($table)->where($pid_field, $pid)->select()->toArray();
...
if (!function_exists('get_plugin_validate')) {
	 ...
	 * @return bool
	 */
	function get_plugin_validate($name = ''): object {
...
if (!function_exists('plugin_url')) {
	...
	function plugin_url($url = '', $param = [], $module = 'admin') {
		...
		// 返回url地址
		return (string) url($module . '/plugin/execute', $params);
	}
}
			...
			if (!empty($action_info['log'])) {
				if (preg_match_all('/\[(\S+?)\]/', $action_info['log'], $match)) {
					$log = [
						...
						'data' => ['user' => $user_id, 'model' => $model, 'record' => $record_id, 'time' => date('Y-m-d H:i:s', request()->time())],
						'details' => $details,
					];
...
if (!function_exists('execute_action')) {
	...
	function execute_action($rules = false, $action_id = null, $user_id = null) {
		...
		foreach ($rules as $rule) {
			...
			$exec_count = model('admin/log')->where($map)->whereTime('create_time', '>', request()->time() - intval($rule['cycle']) * 3600)->count();
...
if (!function_exists('home_url')) {
	 ...
	 * @return string
	 */
	function home_url(string $url = '', array $vars = [], $suffix = true, $domain = false) {
		$url = (string) url($url, $vars, $suffix, $domain);
		if (request()->import == 'admin') {
...
if (!function_exists('admin_url')) {
	...
	function admin_url(string $url = '', array $vars = [], $suffix = true, $domain = false) {
		...
		if (request()->import == 'admin') {
			...
			return preg_replace('/\/index.php/', '/' . request()->import_file, $url);
		}
	}
}
...
if (!function_exists('extend_form_item')) {
	...
	function extend_form_item($form = [], $_layout = []) {
		...
		} elseif (isset($form['view']) && file_exists($form['view'])) {

			$template_content = file_get_contents($form['view']);
...
if (!function_exists('is_disable_func')) {
	...
	function is_disable_func($func = '') {
		...
		$disable_functions = config('zbuilder.disable_functions');
...
function kill_process($pid) {
	...
	trace($last_line);
	return $last_line === false ? false : true;
}

function process_runing_by_pid($pid) {
	if ($pid == 0) {
		return false;
	}
