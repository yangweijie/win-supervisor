...
class Packet extends Model
{
        ...
        foreach ($info['tables'] as $table) {
            ...
            if (file_exists($sql_file)) {
                if (isset($info['database_prefix']) && $info['database_prefix'] != '') {
                    $sql_statement = Sql::getSqlFromFile($sql_file, false, [$info['database_prefix'] => config('database.connections')[config('database.default')]['prefix']]);
                } else {
    ...
    public static function uninstall($name = '')
    {
        $info = self::getInfoFromFile($name);
        foreach ($info['tables'] as $table) {
            $sql = "DROP TABLE IF EXISTS `". config('database.connections')[config('database.default')]['prefix'] ."{$table}`;";
            Db::execute($sql);
