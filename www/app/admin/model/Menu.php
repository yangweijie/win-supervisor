...
class Menu extends Model
{
	...
	public static function getTopMenu($max = '', $cache_tag = '')
	{
		...
		if (!$menus) {
			...
			$list_menu     = self::where($map)->order('sort,id')->column('id,pid,module,title,url_value,url_type,url_target,icon,params', 'id');
			$i             = 0;
			$menus         = [];
			...
			foreach ($list_menu as $key => &$menu) {
				...
				if ($menu['url_value'] != '' && ($menu['url_type'] == 'module_admin' || $menu['url_type'] == 'module_home')) {
					...
					$menu['action']     = $url[2];
					$menu['params']     = $menu['params'] ? json_decode($menu['params'], true) : [];
	...
	public static function getLocation($id = '', $del_last_url = false, $check = true)
	{
		...
		if (!$location) {
			...
			if ($check && empty($location)) {
				Jump::getInstance()->error('获取不到当前节点地址，可能未添加节点');
