...
class Admin extends Common
{
	...
	protected function initialize()
	{
		parent::initialize();
		// 是否拒绝ie浏览器访问
		if (config('zbuilder.deny_ie') && get_browser_type() == 'ie') {
			return redirect('admin/ie/index');
		}

		// 判断是否登录，并定义用户ID常量
		session('UID', $this->isLogin());
		// 设置当前角色菜单节点权限
	...
	{
		// 获取表单项
		$cache_name = app()->http->getName() . '/' . parse_name($this->request->controller()) . '/add';
	...
	public function edit($id = '')
	{
		...
		// 获取表单项
		$cache_name = app()->http->getName() . '/' . parse_name($this->request->controller()) . '/edit';
		$cache_name = strtolower($cache_name);
		$form       = Cache::get($cache_name, []);
		if (!$form) {
	...
	public function setStatus($type = '', $record = [])
	{
		...
		$protect_table = [
			...
			config('database.connections')[config('database.default')]['prefix'] . 'admin_user',
			config('database.connections')[config('database.default')]['prefix'] . 'admin_role',
			config('database.connections')[config('database.default')]['prefix'] . 'admin_module',
		];
