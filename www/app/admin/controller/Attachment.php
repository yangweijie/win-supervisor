...
class Attachment extends Admin
{
		...
		foreach ($data_list as $key => &$value) {
			if (in_array(strtolower($value['ext']), ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) {
				if ($value['driver'] == 'local') {
					...
					$value['type'] = '<div class="js-gallery"><img class="image" title="点击查看大图" data-original="' . Env::get('public_path') . $value['path'] . '" src="' . Env::get('public_path') . $thumb . '"></div>';
				} else {
					$value['type'] = '<div class="js-gallery"><img class="image" title="点击查看大图" data-original="' . $value['path'] . '" src="' . $value['path'] . '"></div>';
				}
			} else {
				if ($value['driver'] == 'local') {
					$path = Env::get('public_path') . $value['path'];
				} else {
					$path = $value['path'];
				}
	...
	private function saveFile($dir = '', $from = '', $module = '')
	{
		...
		if ($file_exists = AttachmentModel::where(['md5' => $file->hash('md5')])->find()) {
			...
			if ($file_exists['driver'] == 'local') {
				$file_path = Env::get('public_path') . $file_exists['path'];
			} else {
		...
		//开始上传	
		try {
			$file_validate[] = ($size_limit == 0) ?: 'fileSize:' . $size_limit;
			$file_validate[] = 'fileExt:' . join(',', $ext_limit);



			$file_validate = join('|', $file_validate);

			validate([$file_input_name => $file_validate])
				->check([$file_input_name => $file]);

			$savename = Filesystem::putFileAs($dir, $file, $file->getOriginalName());
			$file_path = Filesystem::path($savename);
			...
			if ($dir == 'images') {
				...
				if ($watermark == '') {
					if (config('app.upload_thumb_water') == 1 && config('app.upload_thumb_water_pic') > 0) {
						$this->create_water($file_path, config('app.upload_thumb_water_pic'));
					}
				} else {
					if (strtolower($watermark) != 'close') {
						list($watermark_img, $watermark_pos, $watermark_alpha) = explode('|', $watermark);
						$this->create_water($file_path, $watermark_img, $watermark_pos, $watermark_alpha);
					}
				}
				...
				if ($thumb == '') {
					if (config('app.upload_image_thumb') != '') {
						$thumb_path_name = $this->create_thumb($file_path, $dir, $file->getOriginalName());
					}
				} else {
					if (strtolower($thumb) != 'close') {
						list($thumb_size, $thumb_type) = explode('|', $thumb);
						$thumb_path_name = $this->create_thumb($file_path, $dir, $file->getOriginalName(), $thumb_size, $thumb_type);
					}
				}
			}
			...
			$file_info = [
				...
				'path' => str_replace(dirname(public_path()) . DIRECTORY_SEPARATOR . 'public/', '', $file_path),
			...
			if ($file_add = AttachmentModel::create($file_info)) {
				$file_path = Env::get('public_path') . $file_info['path'];
	...
	private function saveScrawl()
	{
		...
		if ($file_add = AttachmentModel::create($file_info)) {
			...
			return json([
				...
				"url" => Env::get('public_path') . $file_info['path'], // 返回的地址
	...
	private function jcrop()
	{
		...
		if ($file_path == '') {
			...
			$savename = Filesystem::putFileAs('temp', $file, md5($file_name));
			$file_path = Filesystem::path($savename);

			if ($savename) {
				return json(['code' => 1, 'src' => Env::get('public_path') . 'uploads/temp/' . md5($file_name)]);
			} else {
				$this->error('上传失败');
			}
		}

		// $file_path = config('app.upload_temp_path') . str_replace(Env::get('public_path') . 'uploads/temp/', '', $file_path);
		...
		if (is_file($file_path)) {
			...
			if ($file_add = AttachmentModel::create($file_info)) {
				...
				return json([
					'code' => 1,
					'id' => $file_add['id'],
					'src' => Env::get('public_path') . $file_info['path'],
					'thumb' => $thumb_path_name == '' ? '' : Env::get('public_path') . $thumb_path_name,
				]);
			} else {
				$this->error('上传失败');
			}
	...
	private function create_thumb($file = '', $dir = '', $save_name = '', $thumb_size = '', $thumb_type = '')
	{
		...
		// 保存缩略图
		$thumb_path = config('app.upload_path') . DIRECTORY_SEPARATOR . $dir . '/thumb/';
		if (!is_dir($thumb_path)) {
			mkdir($thumb_path, 0766, true);
		}
		$thumb_path_name = $thumb_path . $save_name;
		$image->save($thumb_path_name);
		$thumb_path_name = 'uploads/' . $dir . '/thumb/' . $save_name;
