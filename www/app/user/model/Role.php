...
class Role extends Model
{
	...
	public static function checkAuth($id = 0, $url = false)
	{
		...
		// dump(['session(role_menu_auth)' => session('role_menu_auth')]);
		// 检查权限
		if ($menu_auth) {
			if ($id !== 0) {
	...
	public function roleAuth()
	{
		...
		if (!$menu_auth) {
			...
			$menu_auth = $menu_auth ? json_decode($menu_auth, true) : [];
			$menu_auth = MenuModel::where('id', 'in', $menu_auth)->column('url_value','id');
			// dump(['$menu_auth' => $menu_auth]);
		}
		// 非开发模式，缓存数据
	...
	{
		if ($menu_auth) {
			return self::where('menu_auth', 'like', '%"' . $menu_id . '"%')->column('menu_auth');
		} else {
	...
	{
		return self::where('id', 'in', $role)->column('menu_auth');
	}
	...
	{
		if ($pid !== null) {
			$data = self::where('pid', $pid)->column('menu_auth');
