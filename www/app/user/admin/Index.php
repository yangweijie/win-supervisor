...
class Index extends Admin
{
	...
	public function index()
	{
		...
		// dump($data_list->toArray());die;
		// 授权按钮
		$btn_access = [
			'title' => '授权',
			'icon'  => 'fa fa-fw fa-key',
			'href'  => (string)url('access', ['uid' => '__id__'])
		];
		...
		return ZBuilder::make('table')
			...
			->addColumns([ // 批量添加列
				...
				['nickname', '昵称'],
				['role', '角色', $role_list],
				['group', '部门', 'callback', function ($value) {
					$depids =	str_replace(';',',',$value);
					$deps =  Dep::whereIn('id', $depids)->column('title');
					return join(';', $deps);
				}],
				['email', '邮箱'],
				['mobile', '手机号'],
	...
	public function add()
	{
		...
		if ($this->request->isPost()) {
			...
			if ($user = UserModel::create($data)) {
				...
				$this->success('新增成功', (string)url('index'));
			} else {
				$this->error('新增失败');
			}
		}
		...
		return ZBuilder::make('form')
			...
			->addFormItems([ // 批量添加表单项
				...
				['orgtree', 'group', '所在部门', '可在多个部门任职', '', url('dep/get_dps')],
	...
	public function edit($id = null)
	{
		...
		// 获取数据
		$info = UserModel::where('id', $id)->withoutField('password')->find();
		...
		return ZBuilder::make('form')
			...
			->setFormData($info->toArray()) // 设置表单数据
			->fetch();
	}
	...
	public function access($module = '', $uid = 0, $tab = '')
	{
		...
		if ($list_module) {
			...
			foreach ($access as $key => $value) {
				$access[$key]['url'] = (string)url('access', [
					'module'  => $module,
					'uid'     => $uid,
					'tab'     => $key
				]);
			...
			if ($this->request->isPost()) {
				...
				if (isset($post['nodes'])) {
					...
					$this->success('操作成功', (string)url('access', ['uid' => $post['uid'], 'module' => $module, 'tab' => $tab]));
				} else {
					// 清除所有数据授权
					$map['module'] = $post['module'];
					$map['tag']    = $post['tag'];
				...
				if (isset($curr_access_nodes['model_name']) && $curr_access_nodes['model_name'] != '') {
					...
					$nodes = Db::name($curr_access_nodes['table_name'])->order($curr_access_nodes['primary_key'])->field($fields)->select()->toArray();
					$tree_config = [
				...
				$node_access = AccessModel::where($map)->select()->toArray();
				$user_access = [];
