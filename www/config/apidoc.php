<?php
$AppApiDocConfig = getApiDocConfig();
$ApiDocConfig = [
	// （选配）文档标题，显示在左上角与首页
	'title'              => 'Apidoc',
	// （选配）文档描述，显示在首页
	...
	'auto_url' => [
		...
		'custom' => function ($path, $method) {
			...
			return strtolower('/api.php/' . join('/', explode('\\', $path)) . '/' . $method);
		}
	],
	// 是否自动注册路由
...
return array_merge_recursive($ApiDocConfig, $AppApiDocConfig);
...
// 		'actions' => []
// 	],
// ], getAppConfigOfFile(basename(__FILE__)));
