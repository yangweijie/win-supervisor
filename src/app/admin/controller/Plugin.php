...
class Plugin extends Admin
{
	...
	public function install($name = '')
	{
		...
		if (file_exists($sql_file)) {
			if (isset($plugin->database_prefix) && $plugin->database_prefix != '') {
				...
				$sql_statement = Sql::getSqlFromFile($sql_file, false, [$plugin->database_prefix => config('database.connections')[config('database.default')]['prefix']]);
			} else {
	...
	public function uninstall($name = '')
	{
		...
		if (file_exists($sql_file)) {
			if (isset($plugin->database_prefix) && $plugin->database_prefix != '') {
				$sql_statement = Sql::getSqlFromFile($sql_file, true, [$plugin->database_prefix => config('database.connections')[config('database.default')]['prefix']]);
			} else {
	...
	public function add($plugin_name = '')
	{
		...
		if (plugin_action_exists($plugin_name, 'Admin', 'add')) {
			...
			return plugin_action($plugin_name, 'Admin', 'add');
		}
	...
	public function edit($id = '', $plugin_name = '')
	{
		...
		if (plugin_action_exists($plugin_name, 'Admin', 'edit')) {
			...
			return plugin_action($plugin_name, 'Admin', 'edit');
		}
	...
	public function config($name = '')
	{
		...
		// 插件配置项
		if (file_exists(config('app.plugin_path') . $name . '/config.php')) {
	...
	public function execute()
	{
		...
		return plugin_action($plugin, $controller, $action);
	}
