...
class Module extends Model
{
	...
	public function getAll($keyword = '', $status = '')
	{
		...
		if (!$result) {
			$dirs = array_map('basename', glob(root_path() . 'app/' . '*', GLOB_ONLYDIR));
			if ($dirs === false || !file_exists(root_path() . 'app/')) {
			...
			foreach ($modules as &$module) {
				...
				switch ($module['status']) {
					...
					case '-1': // 未安装
						...
						$module['actions'] = '<a class="btn btn-sm btn-noborder btn-success" href="' . (string)url('install', ['name' => $module['name']]) . '">安装</a>';
						$module['status_class'] = 'text-info';
						$module['status_info'] = '<i class="fa fa-fw fa-th-large"></i> 未安装';
						break;
					case '0': // 禁用
						$module['bg_color'] = 'warning';
						$module['actions'] = '<a class="btn btn-sm btn-noborder btn-success ajax-get confirm" href="' . (string)url('enable', ['ids' => $module['id']]) . '">启用</a> ';
						$module['actions'] .= '<a class="btn btn-sm btn-noborder btn-primary" href="' . (string)url('export', ['name' => $module['name']]) . '">导出</a> ';
						$module['actions'] .= '<a class="btn btn-sm btn-noborder btn-danger" href="' . (string)url('uninstall', ['name' => $module['name']]) . '">卸载</a> ';
						$module['status_class'] = 'text-warning';
						$module['status_info'] = '<i class="fa fa-ban"></i> 已禁用';
						break;
					case '1': // 启用
						$module['bg_color'] = 'success';
						$module['actions'] = '<a class="btn btn-sm btn-noborder btn-info ajax-get confirm" href="' . (string)url('update', ['name' => $module['name']]) . '">更新</a> ';
						$module['actions'] .= '<a class="btn btn-sm btn-noborder btn-warning ajax-get confirm" href="' . (string)url('disable', ['ids' => $module['id']]) . '">禁用</a> ';
						$module['actions'] .= '<a class="btn btn-sm btn-noborder btn-primary" href="' . (string)url('export', ['name' => $module['name']]) . '">导出</a> ';
						$module['actions'] .= '<a class="btn btn-sm btn-noborder btn-danger" href="' . (string)url('uninstall', ['name' => $module['name']]) . '">卸载</a> ';
						$module['status_class'] = 'text-success';
						$module['status_info'] = '<i class="fa fa-check"></i> 已启用';
	...
	public static function getInfoFromFile($name = '')
	{
		...
		if ($name != '') {
			// 从配置文件获取
			$info_file_path = root_path() . 'app/' . $name . '/info.php';
			if (is_file($info_file_path)) {
				$info = include $info_file_path;
			}
		}
	...
	{
		$name = $name == '' ? app()->http->getName() : $name;
	...
	{
		$menus = [];
		if ($name != '' && is_file(root_path() . 'app/' . $name . '/menus.php')) {
			// 从菜单文件获取
			$menus = include root_path() . 'app/' . $name . '/menus.php';
		}
		return $menus;
	}
}
