...
class Attachment extends Model
{
    ...
    {
        if (is_array($id)) {
            $data_list = $this->where('id', 'in', $id)->select()->toArray();
            $paths = [];
            foreach ($data_list as $key => $value) {
                if ($value['driver'] == 'local') {
                    $paths[$value['id']] = ($type == 0 ? Env::get('public_path') : '').$value['path'];
                } else {
            ...
            if ($data) {
                if ($data['driver'] == 'local') {
                    return ($type == 0 ? Env::get('public_path') : '').$data['path'];
                } else {
                    return $data['path'];
                }
            } else {
    ...
    public function getThumbPath($id = '')
    {
        ...
        if ($result) {
            if ($result['driver'] == 'local') {
                return $result['thumb'] != '' ? Env::get('public_path').$result['thumb'] : Env::get('public_path').$result['path'];
            } else {
