...
function check_env()
{
	$items = array(
		...
		'php'     => array('PHP版本', '7.4+', '8.0+', PHP_VERSION, 'check'),
...
function write_config($config)
{
	if (is_array($config)) {
		...
		try {
			...
			$Env = new DotEnv(root_path() . '.env');

			foreach ($db_config_keys as $key => $value) {

				$Env->add($key, $db_config[$value]);
			}
