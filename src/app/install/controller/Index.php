...
class Index extends BaseController
{
    ...
    {
        if (session('step') != 1 && session('step') != 3) redirect($this->request->baseFile());
        if (session('reinstall')) {
            session('step', 2);
            redirect($this->request->domain() . '/install/index/step4.html');
        } else {
            session('error', false);
    ...
    public function step3()
    {
        ...
        if ($this->request->isAjax()) {
            if (session('error')) {
                ...
                $this->success('恭喜您环境检测通过', '/install/index/step3.html');
            }
        }
        if (session('step') != 2) redirect($this->request->baseFile());
        session('error', false);
        session('step', 3);
        return View::fetch();
    }
    ...
    public function step4($db = null, $cover = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $database['connections']['mysql']['hostname'] = $db['hostname'];
            ...
            $database['connections']['mysql']['hostport'] = $db['hostport'];
            $database['connections']['mysql']['prefix'] = $db['prefix'];
            // 缓存数据库配置
            session('db_config', $database);;
            Config::set($database, 'database');
            // 防止不存在的数据库导致连接数据库失败
            $db_name =  $database['connections']['mysql']['database'];
            unset($database['connections']['mysql']['database']);
            ...
            // 跳转到数据库安装页面
            $this->success('参数正确开始安装', '/install/index/step4.html');
        } else {
            if (session('step') != 3 && !session('reinstall')) {
                redirect($this->request->baseFile());
            }
    ...
    {
        if (session('step') != 4) {
            $this->error('请按步骤安装系统', request()->domain() . '/install/index/index.html');
        }

        if (session('error')) {
            $this->error('安装出错，请重新安装！', request()->domain() . '/install/index/index.html');
        } else {
