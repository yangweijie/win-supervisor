...
trait UploadTrait
{
    ...
    public function uploadRich()
    {
        ...
        if (!$file) {
            return $this->response()->additional(['errno' => 1])->fail(lang('upload_file_error'));
        }

        $path = $file->store(Admin::config('upload.directory.rich'), Admin::config('upload.disk'));

        $link = Storage::disk(Admin::config('upload.disk'))->url($path);
    ...
    protected function upload($type = 'file')
    {
        ...
        if (!$file) {
            return $this->response()->fail(lang('upload_file_error'));
        }

        $path = $file->store(Admin::config('upload.directory.' . $type), Admin::config('upload.disk'));
