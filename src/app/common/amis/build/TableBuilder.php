 ...
 */
class TableBuilder extends ZBuilder
{
	...
	private $_vars = [
		...
		'column_width'       => [],       // 列宽度
		'column_hide'        => [],       // 隐藏列
		'table_data_empty_tips' => '还没有任何数据 !', // 表格空数据提示
		'quickSaveApi'       => null, // 表格空数据提示
	];

	private $_page = null;

	private $CRUDTable = null;
	...
	public function initialize()
	{
		...
		$this->_template   = root_path() . 'app\common\amis\build\layout.html';

		// 默认加载快速编辑所需js和css
		$this->_vars['_js_files'][]  = 'editable_js';
		$this->_vars['_css_files'][] = 'editable_css';

		$this->_page = amisMake()->page();
	 ...
	 * @return $this
	 */
	public function addTopButton($type = '', $attribute = [], $pop = false, $extra = [])
	{
		if ($type == '') {
			return $this;
		}
		...
		$btn_style = array_merge(['size' => 'lg', 'style' => ''], $extra);
		...
		switch ($type) {
			...
			case 'enable':
				...
				$btn_attribute = [
					...
					'api'  => $this->getDefaultUrl($type, ['ids' => '__id__', '_t' => $table_token, 'field' => $field]),
				];
				break;
			...
			case 'delete':
				...
				$btn_attribute = [
					...
					'size' => $btn_style['size'],
					'actionType' => 'ajax',
					'confirmText' => isset($attribute['data-title']) ?: '确认要删除吗？',
					'class' => 'btn btn-' . $btn_style['size'] . ' btn-' . $btn_style['style'] . ' ajax-get confirm',
					'href'  => $this->getDefaultUrl($type, ['ids' => '__id__', '_t' => $table_token]),
					'api'   => $this->getDefaultUrl($type, ['ids' => '__id__', '_t' => $table_token])
				];
				break;
		...
		switch ($type) {
			...
			case 'edit':
				...
				$btn_attribute = [
					...
					'api'  => $this->getDefaultUrl($type, ['id' => '__id__', 'plugin_name' => $plugin_name]),
	...
	{
		if (!empty($fields)) {
			$this->_vars['search_button'] = is_string($fields) ? explode(',', $fields) : $fields;
	 ...
	 * @return $this
	 */
	public function addColumn($name = '', $title = '', $type = 'text', $default = '', $param = '', $class = '', $extra = '')
	{
		$field = $name;
		$table = '';
	...
	/**
	 * 设置表格数据列表
	 * @param array|object|Closure $row_list 表格数据
	 * @author 蔡伟明 <314013107@qq.com>
	 * @return $this
	 */
	public function setRowList($row_list = null)
	{
		if ($row_list instanceof Closure) {
			$row_list = $row_list();
		}
		if ($row_list !== null) {
			// 原始表格数据
	...
	private function compileRows()
	{
		//items
		$this->_vars['row_list'];
		$row_list = (isset($this->_vars['row_list']['items']) ? $this->_vars['row_list']['items'] : $this->_vars['row_list']);
					...
					switch ($column['type']) {
						case 'link': // 链接
							if ($column['default'] != '') {
								...
								if ($column['extra'] != '') {
									$title = $column['extra'] === true ? $column['label'] : $column['extra'];
								} else {
						...
						case 'text.edit': // 可编辑的单行文本
							...
							//         class="text-edit" 
							//         data-placeholder="请输入' . $column['label'] . '" 
		...
		if (isset($this->_vars['row_list']['items'])) {
			$this->_vars['row_list']['items'] = $row_list;
		} else {
	 ...
	 * 设置数据表快速编辑
	 */
	public function quickSaveApi($url = '', $method = 'post')
	{
		if ($url !== '') {
			$this->_vars['quickSaveApi'] = ['url' => $url, 'method' => $method];
		}
	 ...
	 * 处理数据表搜索
	 */
	private function search_button($columns)
	{
		//设置列搜索 需自行接收参数处理数据返回排序数据
		if (in_array($columns['name'], $this->_vars['search_button']) || in_array($columns['name'], array_keys($this->_vars['search_button']))) {
	...
	{
		// dump($this->_vars['columns']);
		$table_columns_name_list = ['label', 'name', 'type', 'width', 'remark', 'fixed', 'popOver', 'copyable', 'style', 'innerStyle', 'sortable', 'searchable', 'filterable', 'quickEdit', 'quickEditEnabledOn'];
		$table_columns_type_list = ['text', 'image', 'date', 'progress', 'status', 'switch', 'mapping', 'list'];
		$resColumns = [];
		foreach ($this->_vars['columns'] as $key => &$columns) {

			$columns = $this->search_button($columns);

			//设置列排序，需自行接收参数处理数据返回排序数据
			if (in_array($columns['name'], $this->_vars['order_columns'])) {
			...
			switch ($columns['type']) {
				case 'text.edit':
					$columns['type'] = 'text';
					$columns['quickEdit'] = true;
					break;
				case 'btn':
					$columns = [
						'type' => 'operation',
						'label' => $columns['label'],
						'buttons' => $this->_vars['right_buttons'],
					];
					break;
				default:
			}
			// 	dump([$columns, $columns['name'], $columns['type']]);
			// 	$column_names = [];
			// 	foreach (array_keys($columns) as  $column_name) {
			// 		if (in_array($column_name, $table_columns_name_list)) {
			// 		}
			// 	}

			// 	if (in_array($columns['name'], $table_columns_name_list) && in_array($columns['type'], $table_columns_type_list)) {
			// 		dump($columns);
			// 		$resColumns[$key] = $columns;
			// 	}
		}
		dump($this->_vars['columns']);
		return	$this->_vars['columns'];
	}
	/**
	 * 编译表格数据
	 * @author 蔡伟明 <314013107@qq.com>
	...
	private function compileTable()
	{
		...
		if ($this->_vars['quickSaveApi'] == null) {
			$this->_vars['quickSaveApi'] = (string)url('quickEdit');
		}
		// dump($this->_vars['top_buttons']);
		$this->_page->title($this->_vars['page_title'])
			->subTitle($this->_vars['page_tips_top'])
			->body(
				[
					[
						'type' => 'button-group',
						'buttons' => $this->_vars['top_buttons'],
		...
		View::assign('jsonpage', $this->_page->toJson());
	}
	private function CRUDTable()
	{
		...
		//设置数据表快速编辑保存接口
		$this->CRUDTable->quickSaveApi($this->_vars['quickSaveApi']);
		//设置数据分页设置
		$this->CRUDTable->data($this->pageSeting);
		//家长数据
		$this->CRUDTable->loading(true);
		//是否固定表头(table 下)	
		$this->CRUDTable->affixHeader(true);
		//是否将过滤条件的参数同步到地址栏	
		$this->CRUDTable->syncLocation(false);
		//顶部工具栏配置	
		$this->CRUDTable->headerToolbar($this->CRUDTableHeader);
		//底部工具栏配置	
		$this->CRUDTable->footerToolbar($this->CRUDTableFooter);
		//设置列格式数据
		$this->CRUDTable->columns($this->columns());
		//表格数据为空的提示语
		return $this->CRUDTable->placeholder($this->_vars['table_data_empty_tips']);
	}
	/**
	 * 表格数据为空的提示语
	 * @param string $tips 模板文件名
	 * @author 李世平 <dragon_lhp@163.com>
	 * @return mixed
	 */
	public function placeholder($tips = '')
	{
		if ($tips !== '') {
			$this->_vars['table_data_empty_tips'] = $tips;
		}
		return $this;
	}
	...
	public function fetch($template = '', $vars = [])
	{
		if (Request::isJson()) {
			// 编译表格数据row_list的值
			$this->compileRows();
			$row_list = $this->_vars['row_list'];

			if ($row_list) {
				$data =	['status' => 0, 'msg' => 'ok', 'data' => $row_list,];
			} else {
				$data =	['status' => 404, 'msg' => '返回数据错误', 'data' => [],];
			}
		...
		return View::fetch($this->_template, $vars);
	}
}
