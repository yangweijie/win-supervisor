...
class Index extends Admin
{
	...
	public function index()
	{
		...
		return ZBuilder::make('table')
			...
			->setRowList($data_list['data']) // 设置表格数据
	...
	public function add()
	{
		...
		if ($this->request->isPost()) {
			...
			$CrontaResult =   CrontabModel::create($data);
			if ($CrontaResult) {
...
EOF;
	...
	public function edit($id = null)
	{
		...
		if ($this->request->isPost()) {
			...
			$CrontaResult = CrontabModel::where('id', $id)->find();
			$CrontaResult = $CrontaResult->update($data);
			if ($CrontaResult) {
		...
		// 获取数据
		$info = CrontabModel::where('id', $id)->find();
...
EOF;
		...
		return ZBuilder::make('form')
			...
			->addFormItems([ // 批量添加表单项
				...
				['textarea', 'content', '内容', ''],
				['text', 'schedule', '执行周期', '请使用 <code>Cron</code> 表达式<code>', '* * * * *', [], 'style="font-size:12px;font-family: Verdana;word-spacing:23px;"'],
	...
	public function disable($record = [])
	{
		...
		$CrontaResult =   CrontabModel::where(['id' => ['in', $ids]])->update(['status' => 'disable']);
		 
		if (false !== $CrontaResult) {
	...
	public function enable($record = [])
	{
		...
		$CrontaResult =   CrontabModel::where(['id' => ['in', $ids]])->update(['status' => 'normal']);
		if (false !== $CrontaResult) {
			return $this->success('操作成功');
		} else {
			return $this->error('操作失败');
