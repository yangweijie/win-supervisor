...
class Tree
{
	...
	protected static $config = [
		...
		'html'  => '┝ ',   // 层级标记
		'step'  => 4,       // 层级步进数量
		'child_end_empty' => false, //是否在节点最后添加空子数组
		'is_title_prefix' => true, //是否输出 title_prefix
		'is_title_display' => true, //是否输出 title_display
		'display_fields' => [], //输出显示的字段，空为显示所有字段
	];
	...
	public static function toLayer($lists = [], $pid = 0, $max_level = 0, $curr_level = 0)
	{
		...
		foreach ($lists as $key => $value) {
			if ($value[self::$config['pid']] == $pid) {
				...
				if (!empty(self::$config['display_fields'])) {
					$values = [];
					foreach ($value as $k => $item) {
						if (in_array($k, self::$config['display_fields'])) {
							$values[$k] = $item;
						}
					}
					$value = $values;
				...
				} elseif (self::$config['child_end_empty'] == true) {
					$value[self::$config['child']] = [];
				}
				$trees[] = $value;
			}
		}
		...
		if (is_array($lists)) {
			...
			foreach ($lists as $key => $value) {
				if ($value[self::$config['pid']] == $pid) {
					...
					$value['level'] = $level + 1;
					if (self::$config['is_title_prefix']) {
						$value['title_prefix']  = $level == 0 ? '' : $title_prefix;
					}
					if (self::$config['is_title_display']) {
